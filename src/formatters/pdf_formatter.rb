...
class PDFFormatter
  ...
  def format(data)
    ...
    return unless data.class == StationBoardResponse
    ...
    tt.draw_table({ column_widths: [110, 50, 150, 2940, 50] }) do
      data.connections.map do |connection|
        ...
        [DateTime.parse(connection.time).strftime('%k %M'), connection.line, connection.terminal.name, connection.subsequent_stops.map do |s|
          s.name
        end.join(', ').to_s, track]
      end
    end
  ...
  def create_response(filename)
    { 'status' => 'created', 'file' => App.instance[:bucket_service].get_presigned_url(filename),
      'validity_duration' => App.instance[:config]['storage']['signed_url_expiration_time'] }.to_json
  end
end
